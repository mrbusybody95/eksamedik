    ...
    try:
        ...
        for idx, (base_dir, fp) in enumerate(txt_files, 1):
            ...
            try:
                ...
                # ringkasan kebocoran
                total_sisa = sum(
                    v for k, v in counts.items() if k.startswith("sisa_")
                )
        ...
        progress_q.put(f"Output: {output_dir}")
...
class AnonymizerApp:
    ...
    def _build_output_panel(self, p: Frame) -> None:
        self._section_label(p, "💾 Folder Output")
    ...
    def _build_right_panel(self, parent: ttk.Frame) -> None:
        ...
        vsb_leak = ttk.Scrollbar(tab_leak, orient="vertical")
        vsb_leak.pack(side=RIGHT, fill=Y)
        self.leak_tree = ttk.Treeview(
            tab_leak,
            columns=("file", "kategori", "jumlah"),
            show="headings",
            yscrollcommand=vsb_leak.set,
        )
        self.leak_tree.heading("file",     text="File")
        self.leak_tree.heading("kategori", text="Kategori Sisa")
        self.leak_tree.heading("jumlah",   text="Jumlah")
        self.leak_tree.column("file",      width=320)
        self.leak_tree.column("kategori",  width=280)
        self.leak_tree.column("jumlah",    width=70, anchor="center")
    ...
    def _on_done(self, results: list[ProcessResult]) -> None:
        ...
        for r in results:
            for k, v in r.counts.items():
                if k.startswith("sisa_") and v > 0:
                    self.leak_tree.insert("", END, values=(r.file_rel, k, v))

        self.check_btn.config(state="normal")
        total_sisa = sum(
            v for r in results for k, v in r.counts.items() if k.startswith("sisa_")
        )
        if total_sisa > 0:
            messagebox.showwarning(
                "Perhatian — Sisa kebocoran",
                f"Ditemukan {total_sisa} kemungkinan sisa identitas yang belum termasking.\n"
                "Lihat tab 'Ringkasan Kebocoran' untuk detail."
            )
        else:
    ...
    def _show_leak_summary(self) -> None:
        total = sum(
            v for r in self.last_results for k, v in r.counts.items() if k.startswith("sisa_")
        )
        if total == 0:
            ...
            messagebox.showwarning(
                ...
                "Lihat tab 'Ringkasan Kebocoran' dan buka file output\n"
