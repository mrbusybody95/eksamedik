...
# Kesan harus diawali label eksplisit di awal baris — cegah header/nama pasien masuk
_RADIO_KESAN_RE = re.compile(
    r"^[ \t]*(?:Kesan|Kesimpulan|Interpretasi|Conclusion)\s*[:\-]\s*"
    r"(.{5,400}(?:\n(?![ \t]*(?:Kesan|Kesimpulan|Tanggal|Dokter|Nama|No\.|={3})).{0,300}){0,5})",
    re.I | re.MULTILINE,
)
_RADIO_DATE_RE = re.compile(
    r"(?:Tanggal|Tgl\.?)\s*(?:Pemeriksaan)?\s*[:\-]?\s*" + _DATE_RE.pattern, re.I
)
# Jenis radiologi dari field header dokumen rad_*
_RADIO_JUDUL_RE = re.compile(
    r"^[ \t]*(?:Jenis\s*Pemeriksaan|Pemeriksaan|Modalitas)\s*[:\-]\s*(.{3,80})",
    re.I | re.MULTILINE,
)
...
_LAB_STOPNAMES = {
    ...
    "print", "copy", "halaman", "no", "nomor", "laboratorium", "cell",
    "counter", "hematologi", "kimia", "klinik", "ruangan", "bangsal",
}

# ── mapping nama file → peran ────────────────────────────────────────────────
def _file_role(stem: str) -> str:
    """
    Tentukan peran file berdasarkan stem (nama file tanpa ekstensi).
    Return: 'lab' | 'radiologi' | 'cppt_igd' | 'cppt_ranap' | 'resume' | 'unknown'
    """
    s = re.sub(r"_anonim$", "", stem.lower())
    if re.match(r"lab", s):
        return "lab"
    if re.match(r"rad", s):
        return "radiologi"
    if re.search(r"cppt.*(igd|ugd|er\b)", s):
        return "cppt_igd"
    if re.search(r"cppt.*(ranap|inap|rawat)", s):
        return "cppt_ranap"
    if re.match(r"resume|ringkasan|discharge|lembar_resume", s):
        return "resume"
    if re.match(r"cppt", s):
        return "cppt_ranap"
    return "unknown"
...
def extract_lab(text: str) -> dict:
    """
    Ekstrak item laboratorium dari file lab_*.txt.
    Strategi:
    1. Parse tabel kolom (Nama | Nilai | Satuan) — format paling umum di lab RS
    2. Fallback: pola regex per baris
    3. Filter ketat: nama tidak boleh mengandung kata identitas/header
    """
    tanggal_lab: list[str] = [m.group(1) for m in _LAB_DATE_CTX_RE.finditer(text)]
    items: list[dict] = []
    seen: set[str] = set()

    # ── Strategi 1: tabel tab/spasi banyak ─────────────────────────────────
    # Contoh baris: "Hemoglobin          10.2     g/dL"
    # atau:         "Hemoglobin : 10.2 g/dL"
    _ROW_RE = re.compile(
        r"^[ \t]*([A-Za-z][A-Za-z0-9 \(\)/\-\.]{2,50}?)"  # nama
        r"[ \t]{2,}|\s*[:\|]\s*"                            # separator (spasi >=2 atau : |)
        r"([\d\.,]+(?:\s*[\-\u2013]\s*[\d\.,]+)?)"         # nilai
        r"(?:[ \t]+((?:[a-zA-Z%\/\u00b5][A-Za-z0-9\/\u00b5%\.]{0,10})))?",  # satuan opsional
        re.MULTILINE,
    )

    # parse lebih sederhana: baris yang punya pola TEKS ... ANGKA (... SATUAN)
    _BARIS_RE = re.compile(
        r"^[ \t]*"
        r"([A-Za-z][A-Za-z0-9 \(\)/\-\.]{2,45}?)"   # nama pemeriksaan
        r"[ \t]*[:\|]?[ \t]+"                         # separator
        r"([\d\.,]+)"                                  # nilai angka
        r"(?:[ \t]+([a-zA-Z%\/][A-Za-z0-9\/\u00b5%\.]{0,12}))?",  # satuan
        re.MULTILINE,
    )

    # kata yang bila muncul di nama → bukan item lab
    _BARIS_STOP_WORDS = re.compile(
        r"\b(nama|no\.?\s*rm|no\.?\s*lab|no\.?\s*reg|tanggal|tgl|pasien|"
        r"ruang|bangsal|dokter|perawat|alamat|umur|usia|jenis\s*kelamin|"
        r"nilai\s*rujukan|satuan|keterangan|catatan|laboratorium|hasil|"
        r"hematologi|kimia|klinik|cell\s*counter|print|copy|halaman|"
        r"penjamin|bpjs|asuransi|rs\s*\w+|rsud|rsia)\b",
        re.I,
    )

    for m in _BARIS_RE.finditer(text):
        nama   = _clean(m.group(1))
        nilai  = _clean(m.group(2))
        satuan = _clean(m.group(3)) if m.group(3) else ""

        # filter: nama terlalu pendek, kata stop, atau mengandung angka panjang
        if len(nama) < 3:
            continue
        if nama.lower().strip() in _LAB_STOPNAMES:
            continue
        if _BARIS_STOP_WORDS.search(nama):
            continue
        # nilai: harus angka (boleh desimal, tapi bukan pure integer >6 digit = nomor RM)
        if not re.match(r"^\d{1,5}([.,]\d+)?$", nilai.replace(" ", "")):
            continue
...
def extract_radiologi(text: str) -> dict:
    """
    Ekstrak data radiologi dari file rad_*.txt.
    - Jenis: dari keyword modalitas ATAU field 'Pemeriksaan/Jenis Pemeriksaan'
    - Tanggal: dari field tanggal pemeriksaan
    - Kesan: hanya dari blok yang diawali label eksplisit 'Kesan:' / 'Kesimpulan:'
      (bukan dari baris acak yang kebetulan mengandung kata 'Hasil')
    """
    # ── jenis pemeriksaan ─────────────────────────────────────────────────
    jenis: list[str] = []
    # cari dari field header dulu (lebih presisi)
    for m in _RADIO_JUDUL_RE.finditer(text):
        v = _clean(m.group(1))
        if v and v not in jenis:
            jenis.append(v)
    # fallback: nama modalitas dalam teks
    if not jenis:
        for m in _RADIO_JENIS_RE.finditer(text):
            v = _clean(m.group(0))
            if v and v not in jenis:
                jenis.append(v)
    ...
    # ── kesan / interpretasi ──────────────────────────────────────────────
    # Hanya tangkap blok sesudah label "Kesan:" atau "Kesimpulan:" di awal baris.
    # Hentikan di baris berikutnya yang merupakan field baru (kapital diikuti ':')
    kesan: list[str] = []
    for m in _RADIO_KESAN_RE.finditer(text):
        raw = _clean(m.group(1))
        # buang baris yang mengandung kata identitas pasien
        _ID_WORDS = re.compile(
            r"\b(nama\s*:|no\.?\s*rm|no\.?\s*lab|pasien|tanggal\s*lahir|"
            r"alamat|nomor\s*rekam|jenis\s*kelamin)\b", re.I
        )
        if _ID_WORDS.search(raw):
            continue
        if len(raw) > 5 and raw not in kesan:
            kesan.append(raw)
...
def extract_all(
    ...
    """
    Routing file-aware: ekstraktor yang dijalankan disesuaikan dengan jenis file.

    Aturan routing (berdasarkan nama file):
      lab_*       → laboratorium saja
      rad_*       → radiologi saja
      cppt_igd*   → onset + staf (tidak klinis ranap)
      cppt_ranap* → klinis (keluhan, diagnosis, terapi) + staf
      resume*     → klinis + staf
      lainnya     → semua target yang dicentang user

    Target yang dicentang user tetap sebagai gate: kalau user matikan Lab,
    file lab_* pun tidak diekstrak.
    """
    result: dict = {"diekstrak_pada": datetime.now().isoformat(timespec="seconds")}
    role = _file_role(file_stem) if file_stem else "unknown"
    result["jenis_file"] = role

    # ── routing per jenis file ────────────────────────────────────────────
    do_klinis = target_klinis and role in ("cppt_ranap", "resume", "unknown")
    do_onset  = target_onset  and role in ("cppt_igd", "unknown")
    do_staf   = target_staf   and role in ("cppt_igd", "cppt_ranap", "resume", "unknown")
    do_lab    = target_lab    and role in ("lab", "unknown")
    do_radio  = target_radio  and role in ("radiologi", "unknown")
...
def run_extraction(
    ...
    try:
        ...
        # ── debug: tampilkan folder yang dicari ──────────────────────────
        progress_q.put(f"[Ekstraksi] Mencari di {len(input_dirs)} folder:")
        for d in input_dirs:
            progress_q.put(f"  📂 {d}")
        ...
        for in_dir in input_dirs:
            found_in_dir = 0
            skipped_in_dir = 0
            for fp in sorted(in_dir.glob("*.txt")):   # glob (non-rekursif) untuk 1 level
                if fp.stem.endswith("_anonim") or fp.stem.endswith("_ekstrak"):
                    skipped_in_dir += 1
                    continue
                txt_files.append((in_dir, fp))
                found_in_dir += 1
            # juga rekursif ke sub-subfolder (jika ada)
            for fp in sorted(in_dir.rglob("*.txt")):
                ...
                found_in_dir += 1
            progress_q.put(
                f"  → {found_in_dir} file ditemukan, {skipped_in_dir} dilewati (_anonim/_ekstrak)"
            )

        progress_q.put(f"[Ekstraksi] Total file .txt akan diproses: {len(txt_files)}")
        if not txt_files:
            progress_q.put("[Ekstraksi] \u26a0 Tidak ada file .txt ditemukan.")
            progress_q.put("  Pastikan folder yang dipilih langsung berisi file .txt")
            progress_q.put("  (mis. pilih STROKE_001/, bukan 02_text_extracted/).")
            result_q.put([])
            return
        ...
        for idx, (base_dir, fp) in enumerate(txt_files, 1):
            ...
            role = _file_role(fp.stem)
            progress_q.put(f"[Ekstraksi] [{idx}/{len(txt_files)}] {rel}  [{role}]")
            try:
...
class AnonymizerApp:
    ...
    def __init__(self) -> None:
        ...
        # ── Ekstraksi ──────────────────────────────────────────────────────
        self.extract_input_var:  StringVar = StringVar()
        self.extract_input_dirs: list[Path] = []
    ...
    def _build_extract_output_panel(self, p: Frame) -> None:
        # ── folder input khusus ekstraksi ────────────────────────────────
        self._section_label(p, "📂 Folder Input")
        self._hint(p, "Otomatis pakai folder yang sama dengan tab Anonimisasi, atau pilih sendiri.")
        row_in = Frame(p, bg=self.COL_CARD)
        row_in.pack(fill=X, pady=(4, 0))
        from tkinter import Entry
        self.ex_in_entry = Entry(
            row_in, textvariable=self.extract_input_var,
            font=self.FONT_SM, fg="#111827",
            highlightthickness=1, highlightbackground="#d1d5db",
            relief="flat", bd=4,
        )
        self.ex_in_entry.pack(side=LEFT, fill=X, expand=True)
        self._btn(row_in, "Pilih", self._choose_extract_input, side=RIGHT, pad=6)

        # ── folder output JSON ────────────────────────────────────────────
        self._section_label(p, "📁 Folder Output JSON")
        self._hint(p, "Setiap file .txt menghasilkan satu file _ekstrak.json.")
        row_out = Frame(p, bg=self.COL_CARD)
        row_out.pack(fill=X, pady=(4, 0))
        self.ex_out_entry = Entry(
            row_out, textvariable=self.extract_output_var,
            font=self.FONT_SM, fg="#111827",
            highlightthickness=1, highlightbackground="#d1d5db",
            relief="flat", bd=4,
        )
        self.ex_out_entry.pack(side=LEFT, fill=X, expand=True)
        self._btn(row_out, "Pilih", self._choose_extract_output, side=RIGHT, pad=6)
    ...
    def _choose_extract_input(self) -> None:
        """Pilih folder input khusus untuk ekstraksi (bisa berbeda dari tab Anonimisasi)."""
        path = filedialog.askdirectory(title="Pilih folder berisi file .txt untuk diekstrak")
        if not path:
            return
        self.extract_input_var.set(path)
        # kumpulkan folder itu sendiri + seluruh subfoldernya
        root = Path(path)
        dirs = [root] + [c for c in sorted(root.rglob("*")) if c.is_dir()]
        self.extract_input_dirs = dirs
        # auto-set output folder
        if not self.extract_output_var.get().strip():
            self.extract_output_var.set(str(root.parent / "04_ekstraksi_json"))
    ...
    def _get_extract_dirs(self) -> list[Path]:
        """
        Resolusi folder input untuk ekstraksi, dengan prioritas:
        1. Folder yang dipilih di tab Ekstraksi (extract_input_dirs)
        2. Folder dari tab Anonimisasi (self.input_dirs)
        Kembalikan daftar folder beserta semua subfolder yang mengandung .txt
        """
        # Cek apakah user sudah pilih folder khusus di tab Ekstraksi
        ex_in = self.extract_input_var.get().strip()
        if ex_in and Path(ex_in).is_dir():
            root = Path(ex_in)
            # sertakan root dan semua subdirektori yang punya .txt
            candidates = [root] + [c for c in sorted(root.rglob("*")) if c.is_dir()]
            return [d for d in candidates if any(d.glob("*.txt"))]

        # Fallback ke folder dari tab Anonimisasi
        anon_dirs = [p for p in self.input_dirs if p.is_dir()]
        if anon_dirs:
            return anon_dirs
    ...
    def _validate_extract_ready(self) -> bool:
        dirs = self._get_extract_dirs()
        if not dirs:
            messagebox.showerror(
                "Folder input kosong",
                "Pilih folder input di tab Ekstraksi, "
                "atau tambahkan folder di tab Anonimisasi terlebih dahulu."
            )
            return False
        out = self.extract_output_var.get().strip()
        if not out:
            default = str(dirs[0].parent / "04_ekstraksi_json")
    ...
    def _run_both(self) -> None:
        ...
        # folder anonimisasi (dari tab Anonimisasi)
        valid_anon = [p for p in self.input_dirs if p.is_dir()]
        if not valid_anon:
        ...
        if not out_anon:
            messagebox.showerror("Output kosong",
                "Tentukan folder output anonimisasi terlebih dahulu.")
            self._both_mode = False
            return
        if not self._validate_extract_ready():
            self._both_mode = False
        ...
        # folder ekstraksi (bisa sama atau berbeda)
        valid_ex = self._get_extract_dirs()
        if not self.extract_output_var.get().strip():
            self.extract_output_var.set(str(valid_anon[0].parent / "04_ekstraksi_json"))
        ...
        self.worker = threading.Thread(
            target=run_pipeline,
            args=(valid_anon, Path(out_anon), self.csv_path, self.progress_q, self.result_q),
            daemon=True,
        )
        self.extract_worker = threading.Thread(
            target=run_extraction,
            args=(valid_ex, out_ex, self.extract_progress_q, self.extract_result_q),
            kwargs=dict(
                target_klinis=self.ex_klinis.get(),
                target_onset=self.ex_onset.get(),
                target_staf=self.ex_staf.get(),
