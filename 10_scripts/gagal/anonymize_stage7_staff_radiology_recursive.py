...
# ============================================================
# ANONIMISASI STAGE 7 - STAFF + RADIOLOGY HEADER CLEANUP
#
# Struktur folder:
# Pipeline RME Stroke/
...
# └── 06_scripts/
#     ├── anonymize_stage7_staff_radiology_recursive.py
#     ├── staff_doctors.csv
#     └── staff_extra.csv       <-- opsional
#
# Jalankan dari folder 06_scripts:
#   python anonymize_stage7_staff_radiology_recursive.py
# ============================================================
...
def anonymize_radiology_admin_header(text: str):
    """
    Tambahan Stage 7:
    Membersihkan header administratif radiologi/lab yang sering bocor
    setelah ekstraksi PDF, misalnya:
    - MEDICAL RECORD NUMBER 00-xx-xx-xx
    - No. Foto
    - Bandung - Jawa Barat
    - sisa token telepon/alamat RS yang menempel
    Fungsi ini tidak menghapus isi klinis seperti 'kesan', 'hasil CT', 'infark',
    'perdarahan', dll.
    """
    counts = {}
    ...
    patterns = [
        (
            "MEDICAL_RECORD_NUMBER_LINE",
            r"(?im)^\s*(?:MEDICAL\s*RECORD\s*NUMBER|Medical\s*Record\s*Number|No\.?\s*Medical\s*Record)\s*[:\-]?\s*[A-Za-z0-9\-\/\. ]{3,80}\s*$",
            "[NO_RM]"
        ),
        (
            "MEDICAL_RECORD_NUMBER_INLINE",
            r"(?i)\b(?:MEDICAL\s*RECORD\s*NUMBER|Medical\s*Record\s*Number|No\.?\s*Medical\s*Record)\s*[:\-]?\s*[A-Za-z0-9\-\/\. ]{3,80}",
            "[NO_RM]"
        ),
        (
            "RAW_RM_DASHED",
        ...
        (
            "NO_REGISTRASI",
            r"(?i)\b(?:No\.?\s*Reg(?:istrasi)?|No\.?\s*Register|No\.?\s*Kunjungan)\s*[:\-]?\s*[A-Za-z0-9\-\/\.]+",
            "[NO_REGISTER]"
        ),
        (
            "LOKASI_RS_BANDUNG_JABAR",
            r"(?i)\bBandung\s*[-,]?\s*Jawa\s*Barat\b\.?",
            "[LOKASI_RS]"
        ),
        (
            "SISA_TELEPON_RS_MENEMPEL",
            r"(?i)\[TELEPON_RS\]\s*[A-Z]*BANDUNG\s*[-,]?\s*JAWA\s*BARAT\.?",
            "[TELEPON_RS] [LOKASI_RS]"
        ),
        (
            "SISA_ALAMAT_RS_MENEMPEL",
            r"(?i)\[ALAMAT_RS\]\s*[A-Z]*BANDUNG\s*[-,]?\s*JAWA\s*BARAT\.?",
            "[ALAMAT_RS] [LOKASI_RS]"
        ),
        (
            "NAMA_RS_AL_ISLAM_VARIAN",
            r"(?i)\b(?:RS|R\.S\.|Rumah\s*Sakit)\s+Al\s+Islam\s+Bandung\b",
            "[RUMAH_SAKIT]"
        ),
        (
            "HEADER_XRAY_EXAMINATION",
            r"(?i)\bRadiologi\s+X\s*RAY\s+EXAMINATION\b",
            "[HEADER_RADIOLOGI]"
        ),
        (
            "HEADER_LEMBAR_HASIL",
            r"(?i)\bLEMBAR\s+HASIL\s+(?:PEMERIKSAAN|RADIOLOGI|LABORATORIUM)\b",
            "[HEADER_HASIL]"
        ),
        (
            "NO_LAB",
            r"(?i)\bNo\.?\s*Lab\s*[:\-]?\s*[A-Za-z0-9\-\/\.]+",
            "[NO_LAB]"
        ),
    ]
    ...
    # Buang baris header administratif yang tidak berguna untuk ekstraksi klinis.
    # Ini lebih aman daripada menghapus paragraf panjang.
    admin_line_patterns = [
        r"(?i)^\s*(?:No\.?\s*Foto|No\.?\s*RM|Medical\s*Record|No\.?\s*Reg|No\.?\s*Lab)\b.*$",
        r"(?i)^\s*(?:Nama\s*Pasien|Tanggal\s*Lahir|Jenis\s*Kelamin|Alamat)\b.*$",
        r"(?i)^\s*(?:Dokter\s*Pengirim|Unit\s*Asal|Ruangan|Penjamin)\b.*$",
        r"(?i)^\s*(?:Dicetak|Tanggal\s*Cetak|Print\s*Copy)\b.*$",
    ]

    for i, pat in enumerate(admin_line_patterns, start=1):
        text, n = re.subn(pat, "", text, flags=re.I | re.M)
        counts[f"ADMIN_LINE_REMOVED_{i}"] = n
...
def anonymize_text(text: str, staff_variants):
    ...
    text, c1b = anonymize_radiology_admin_header(text)
    ...
    text, c6b = anonymize_radiology_admin_header(text)
    ...
    return text, merge_counts(c1, c1b, c2, c3, c4, c5, c6, c6b, c7, c8, c9, c10)
